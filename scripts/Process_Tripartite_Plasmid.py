...
def main():
    df = pd.read_csv("screen_data/traits/tripartite_plasmid_traits.csv")
    tripartite_rules = fqp.df_to_rules(df)
    print(tripartite_rules)
    tripartite_condition = pd.read_csv("screen_data/conditions/tripartite_plasmid.csv").replace(np.nan,None)
