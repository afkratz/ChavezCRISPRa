...
def main():
    ...
    output_sup_table = pd.DataFrame({
        ...
        'Hit on EPCAM':only_centroids['EPCAM 2-fold?'],
        'Hit on CXCR4':only_centroids['CXCR4 2-fold?'],
        'Hit on tdTomato':only_centroids['Reporter 2-fold?'],
    ...
    odf.to_csv(
        os.path.join(
            ...
            "output",
            "prescreen_results",
            "4_PADDLE_precision_recall_data.csv"
        )
        )
